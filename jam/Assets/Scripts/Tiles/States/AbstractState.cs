...
namespace DefaultNamespace
{
    public abstract class AbstractState : ITileState
    {
        ...
        // 2 AC WIN
        // 3 B WIN
        protected int DoubleOneWaySingleAttack(List<IUnit> A, List<IUnit> B, List<IUnit> C) //B ATTACK FIRST
        {
            while (true)
            {
                int resultAB;
         ...
         */
        protected int Versus(List<IUnit> A, List<IUnit> B, bool Afirst)
        {
        ...
        protected int OneWayAttackLoop(List<IUnit> A, List<IUnit> B)
        {
        ...
        {
            IUnit AUnit = thisTile.AttackerAvailable(A);
            IUnit BUnit = thisTile.UnitAlive(B);
